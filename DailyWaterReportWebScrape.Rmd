...
datax <- read.xlsx("DailyWaterReportWebScrape.xlsx", sheetName = "pg1")
datay <- read.xlsx("DailyWaterReportWebScrape.xlsx", sheetName = "pg2")
datax <- datax[,-1]
datay <- datay[,-1]
# Merge
datanewx <- rbind(datax, mxdf, txdf, wxdf, thxdf, fxdf, sxdf, suxdf)
datanewy <- rbind(datay, mydf, tydf, wydf, thydf, fydf, sydf, suydf)
datanewx <- datanewx[!duplicated(datanewx),]
datanewy <- datanewy[!duplicated(datanewy),]
# Save
wb <- createWorkbook(type = "xlsx")
sheet1 <- createSheet(wb, sheetName = "pg1")
sheet2 <- createSheet(wb, sheetName = "pg2")
addDataFrame(datanewx, sheet1, startRow = 1, startColumn = 1)
addDataFrame(datanewy, sheet2, startRow = 1, startColumn = 1)
saveWorkbook(wb, "DailyWaterReportWebScrape.xlsx")
```
